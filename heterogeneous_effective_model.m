...
%The main goal of this script is to extract the effective bifurcation
%parameters. We used a strategy to disentangle the changes in local parameters
%due to network effects from those due to local modifications. 
%This analysis provides information about the origin (local or network-related) 
%of the different dynamics of the ROIs for the different groups.
...
for iter = 1:100
    ...
    k=1;
    for t=1:dt:1000
        suma = wC*z;% To define the effective we change this line and remove - sumC.*z; % sum(Cij*xi) - sum(Cij)*xj
...
bifpar_effective_ord(idx_g,:)=bifpar(idx_g,[1:2:214 2:2:214]);
